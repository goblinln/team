...
require 'scripts/vendor/mysql/mysql_h'
...
local error = function(msg)
	_G.error(debug.traceback(msg, 2), 0);
end
...
local function config(lib)
	if not C then
		if not lib or lib == 'mysql' then
			C = ffi.load(ffi.abi'win' and 'mysql' or 'mysqlclient')
...
function M.connect(t, ...)
	...
	if type(t) == 'string' then
		...
		host, user, pass, db, charset, port = t.host, t.user, t.pswd, t.db, t.charset, t.port
...
function conn.insert_id(...)
	return tonumber(C.mysql_insert_id(...))
end
...
function stmt.insert_id(...)
	return tonumber(C.mysql_stmt_insert_id(...))
end
